...
def login_with_query_string(query_string: str, logger: logging.Logger) -> Optional[Dict[str, Any]]:
...
def claim_daily(query_string: str, logger: logging.Logger, last_claim: Optional[datetime] = None) -> Optional[Dict[str, Any]]:
...
def start_game(level: int, query_string: str, logger: logging.Logger) -> Optional[Dict[str, Any]]:
...
def process_account(query_string: str):
    ...
    while True:
        try:
            user_data = login_with_query_string(query_string, logger)
            if not user_data:
                break
            ...
            claim_response = claim_daily(query_string, logger, last_claim_time)
            if claim_response:
            ...
            while True:
                ...
                game_data = start_game(user_level, query_string, logger)
...
def main():
    init(autoreset=True)
    banner = """Tool được phát triển bởi nhóm tele Airdrop Hunter Siêu Tốc (https://t.me/airdrophuntersieutoc)"""
    print(f'{Fore.YELLOW}{banner}{Fore.RESET}')
    print(f"{Fore.YELLOW}Starting now ...{Fore.RESET}")
    ...
    try:
        for query_string in query_strings:
            thread = threading.Thread(target=process_account, args=(query_string,))
            thread.start()
